...
<div class="relative h-screen w-full overflow-hidden bg-gray-100 font-sans" x-data="bahaMap">
    ...
    <div class="absolute top-6 left-4 right-4 z-[500]">
        <div class="bg-white rounded-xl shadow-lg flex items-center p-3">
            <svg class="w-5 h-5 text-gray-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
        ...
        <div class="mt-2 bg-white rounded-xl shadow-lg overflow-hidden max-h-60 overflow-y-auto border border-gray-100">
            @forelse($filteredHotspots as $spot)
                <div wire:click="selectHotspot({{ $spot->id }}); $set('searchQuery', '')"
                    ...
                    <span class="text-[10px] px-2 py-1 rounded-md font-bold uppercase {{ $spot->status === 'flooded' ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-600' }}">
                        {{ $spot->status }}
                    </span>
                </div>
            @empty
    ...
    <div class="absolute bottom-48 right-4 z-[500] space-y-3 flex flex-col">
        <button @click="locateMe()" class="bg-white text-gray-600 p-3 rounded-xl shadow-lg hover:text-blue-600 transition">
            <svg x-show="!loadingLocation" class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path></svg>
        ...
        <button @click="resetMap()" class="bg-emerald-500 text-white p-3 rounded-xl shadow-lg hover:bg-emerald-600 transition">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg>
        </button>
    </div>
        ...
        <div class="px-6 pb-24 h-full overflow-y-auto">
            @if($selectedHotspot)
                <div class="flex items-center justify-between mb-2">
                    <button wire:click="clearSelection" class="text-xs text-emerald-600 font-bold hover:underline">← ALL ZONES</button>
                    <span class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Updated {{ $selectedHotspot->updated_at->diffForHumans() }}</span>
                </div>
                ...
                <div class="mb-6">
                    @if($selectedHotspot->status === 'flooded')
                        <span class="px-3 py-1 bg-red-100 text-red-600 text-xs font-bold rounded-full">ⓧ IMPASSABLE</span>
                    @elseif($selectedHotspot->status === 'moderate')
                        <span class="px-3 py-1 bg-amber-100 text-amber-600 text-xs font-bold rounded-full">⚠ CAUTION</span>
                    @else
                        <span class="px-3 py-1 bg-emerald-100 text-emerald-600 text-xs font-bold rounded-full">✓ PASSABLE</span>
                    @endif
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-6">
                    ...
                    <div class="bg-indigo-50 p-4 rounded-2xl border border-indigo-100">
                        <div class="flex justify-between items-start mb-1 text-indigo-500">
                            ...
                            <span class="text-[9px] font-bold uppercase">Prev (1h)</span>
                        </div>
                ...
                <div class="bg-gray-50 p-5 rounded-2xl border border-gray-100 space-y-4">
                    <h5 class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Zone Metrics</h5>
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-gray-600 flex items-center gap-2">⛰ Elevation</span>
                        <span class="text-sm font-bold text-gray-800">{{ $selectedHotspot->elevation_m ?? '0' }}m ASL</span>
                    </div>
                    <div class="space-y-2">
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-600 flex items-center gap-2">🛤 Drainage Quality</span>
            ...
            @else
                ...
                <div class="space-y-3">
                    ...
                    <div wire:click="selectHotspot({{ $spot->id }})" @click="detailOpen = true" class="flex items-center justify-between p-4 border border-gray-100 rounded-2xl hover:bg-gray-50 transition cursor-pointer">
...
<script data-navigate-once>
    document.addEventListener('alpine:init', () => {
        Alpine.data('bahaMap', () => ({
            ...
            initMap() {
                ...
                locations.forEach(loc => {
                    ...
                    const icon = L.divIcon({
                        ...
                        html: `<div class="relative flex items-center justify-center w-10 h-10"><div class="absolute w-full h-full rounded-full opacity-60 animate-pulse ${color}"></div><div class="relative w-4 h-4 rounded-full border-2 border-white ${color} z-10"></div></div>`,
            ...
            resetMap() { this.map.flyTo([13.621775, 123.194830], 14); }
        }));
    });
</script>
