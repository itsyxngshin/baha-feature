...
<div class="relative h-screen w-full overflow-hidden bg-gray-100 font-sans" x-data="mapHandler()">
            ...
            @forelse($filteredHotspots as $spot)
                ...
                >
                    <div class="flex items-center">
                        <svg class="w-4 h-4 text-gray-400 group-hover:text-emerald-500 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path></svg>
                        <span class="text-sm font-bold text-gray-700 group-hover:text-emerald-700">{{ $spot->name }}</span>
                    </div>
                    <span class="text-[10px] px-2 py-1 rounded-md font-bold uppercase tracking-wider
                        {{ $spot->status === 'flooded' ? 'bg-red-100 text-red-600' : ($spot->status === 'moderate' ? 'bg-amber-100 text-amber-600' : 'bg-emerald-100 text-emerald-600') }}">
                        {{ $spot->status }}
                    </span>
                </div>
            @empty
    ...
    <div
        class="absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.15)] z-[600] transition-transform duration-300 ease-out transform h-[65vh]"
    ...
    <div class="absolute bottom-0 w-full bg-white border-t border-gray-100 py-2 px-6 flex justify-between items-center z-[700] text-[10px] font-bold text-gray-400 uppercase">
        <a href="#" class="flex flex-col items-center gap-1 hover:text-emerald-600 transition"><div class="w-5 h-5 bg-gray-200 rounded"></div> Home</a>
        <a href="{{ route('baha.map') }}" class="flex flex-col items-center gap-1 text-emerald-600"><div class="w-5 h-5 bg-emerald-100 rounded text-emerald-600 flex items-center justify-center"><svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd"></path></svg></div> Baha Map</a>
        <a href="#" class="flex flex-col items-center gap-1 hover:text-emerald-600 transition"><div class="w-5 h-5 bg-gray-200 rounded"></div> Services</a>
        <a href="#" class="flex flex-col items-center gap-1 hover:text-emerald-600 transition"><div class="w-5 h-5 bg-gray-200 rounded"></div> Profile</a>
    </div>
</div>
...
<script>
    function mapHandler() {
        return {
            ...
            init() {
                // Wrap in timeout to ensure DOM is fully ready before Leaflet fires
                setTimeout(() => {
                    this.initMap();
                }, 100);

                Livewire.on('hotspotsUpdated', () => console.log('Data refreshed'));
            },
